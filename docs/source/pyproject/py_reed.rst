...
In this lesson, we'll explore how to use a **reed switch** with the Raspberry Pi Pico to detect the presence of a magnetic field. A reed switch is a simple electrical switch that operates using a magnetic field. When a magnet comes near the switch, its internal contacts close, completing an electrical circuit.

**Components Needed**

* Raspberry Pi Pico 2
* Reed switch
* Magnet (any small magnet will do)
...
.. note::
  ...
  * Open the ``2.9_feel_the_magnetism.py`` file under the path of ``newton-lab-kit/micropython`` or copy this code into Thonny IDE, then click "Run Current Script" or simply press F5 to run it.
...
Using a reed switch with the Raspberry Pi Pico allows you to detect the presence of a magnetic field, enabling a wide range of applications from security systems to interactive projects. Understanding how to wire the reed switch and utilize interrupts enhances your ability to create efficient and responsive programs.
